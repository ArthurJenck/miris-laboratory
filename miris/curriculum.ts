    ...
    subs: [
      {
        ...
        check: "floor",
        explain:
          "Three nodes and nothing clever. A circle lying flat for the deck, one open-ended cylinder turned inside out for the wall, and six lit panels set into it. The wall earns its place: without something behind them, the capsules sit against pure black, and dark glass over black is invisible. The panels sit between the capsules rather than behind them, on the half-step of the same circle, so the gaps you look through read as a room: they exist to give a dark room a sense of scale, and here they are doing the same job as a lit window at the end of a film set corridor. Each one is a plain unlit plane wearing a texture that fades out at all four edges, because a panel with a hard border reads as a blue rectangle stuck on a wall rather than as light coming through glass. The room is deliberately almost black. Nearly everything you are about to see is emissive geometry rather than lit surfaces, which is why the three lights above the scene block are so dim: a hemisphere for the faintest sense of up and down, one directional to put a gradient across the deck and the wall so they are not flat, and a single cyan point at ankle height under the walkway.",
      },
      {
        num: "2.2",
        title: "The walkway",
      ...
      },
      {
        ...
        check: "capsules",
        explain:
          "The circle is three lines of trigonometry, written out rather than hidden in a helper, because it is the one bit of maths worth reading: an angle of i over six turns, then cosine for x and sine for z. Each capsule is a plinth, a glass cylinder, a cone of light and two rings. The cone is the cheapest volumetric there is: an open cone drawn on both sides with additive blending, so wherever it overlaps itself it brightens, thinning along its length so the beam has no visible end. It writes no depth, which keeps it from cutting a hole in the glass behind it. The cylinder is open-ended and drawn on both sides, which is what makes you see the far wall of the glass through the near one. The glass is a standard material at sixteen percent opacity with a faint emissive of its own, and depthWrite is off so the near wall never hides the far one. That last prop is the one people miss: without it, transparent surfaces punch holes in each other depending on the order they happen to be drawn. Sixteen percent of a flat tint is not what makes it read as glass, though: that comes from the band of light along the silhouette, where you are looking through the curve edge on rather than straight at it. Bubbles draws it, in the same pass as the fluid, because a bright rim is worth having and six more transparent cylinders is not.",
      },
      {
        num: "2.4",
  ...
  },
  {
    ...
    subs: [
      ...
      },
      {
        ...
        body:
          "It sits itself. FitInGlass measures your specimen as it arrives and scales it to the glass, so you do not have to guess a number that is right for one creature and wrong for the next. What is left to you is how much room it takes. Open app/stage.tsx, find fill on the FitInGlass you added at step 2.5, and change it. One is the whole capsule.",
        code: "fill={0.7}",
        explain:
          "A generated mesh arrives at whatever size the generator chose. Measured across a real series it was about 0.28 units, so the fixed scale this step used to ask you to tune by eye put every specimen in the glass at a fortieth of the height it should have had: a speck, and the same speck whatever you grew. The size is not known until the stream loads, so it cannot be a constant. MirisStream.getBounds reports in world space with the current scale already applied, which makes the correction proportional: measure, multiply, measure again, and stop when it stops moving. Fitting to a reported box is only ever as good as the box, which is why the fraction is yours rather than the fit being final.",
      },
      {
        num: "3.4",
  ...
  },
  {
    ...
    subs: [
      ...
      },
      {
        ...
        check: "field",
        explain:
          "TSL is three.js's node shading language. Every call here is a node, not a value: uv() is the pixel's position, time counts up on its own, and sub, mul and length build a graph rather than doing arithmetic now. The whole graph compiles once, to GLSL on this backend, and then runs per pixel per frame. That is also why it is wrapped in useMemo: build it again on every render and you rebuild the renderer with it. Two things are drawn. A ring of light around the specimen under your pointer, and scanlines drifting up the screen. The ring is a distance from the specimen's projected centre, with a radius of its projected half width, so it is zero over the creature itself and brightest just past its edge. An earlier version drew a filled disc there and washed the specimen out. Change the numbers and save: 0.84 and 1.3 are where the ring starts inside the radius and where it fades outside it, 220 is how many scanlines fit the screen, and the vec3 is the colour of the whole field.",
      },
    ],
  },
  {
