  ...
  },
  {
    ...
    subs: [
      ...
      },
      {
        ...
        check: "field",
        explain:
          "TSL is three.js's node shading language. Every call here is a node, not a value: uv() is the pixel's position, time counts up on its own, and sub, mul and length build a graph rather than doing arithmetic now. The whole graph compiles once, to GLSL on this backend, and then runs per pixel per frame. That is also why it is wrapped in useMemo: build it again on every render and you rebuild the renderer with it. Two things are drawn. Scanlines drifting up the screen, and a vignette that darkens the corners. The scanlines are a sine of the pixel's height, sped along by time and squashed to three percent so they read as texture rather than stripes. The vignette is the pixel's distance from the centre of the screen, pushed through smoothstep so nothing happens until the edges and then it falls off gently, and it lands in the alpha with a black colour, so it darkens whatever is behind it instead of adding to it. Change the numbers and save: 220 is how many scanlines fit the screen, 0.8 and 1.6 are where the vignette starts and where it is darkest, and the vec3 is the colour of the lines.",
      },
    ],
  },
  {
