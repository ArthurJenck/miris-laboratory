  ...
  },
  {
    ...
    subs: [
      ...
      },
      {
        ...
        check: "field",
        explain:
          "TSL is three.js's node shading language. Every call here is a node, not a value: uv() is the pixel's position, time counts up on its own, and sub, mul and length build a graph rather than doing arithmetic now. The whole graph compiles once, to GLSL on this backend, and then runs per pixel per frame. That is also why it is wrapped in useMemo: build it again on every render and you rebuild the renderer with it. Two things are drawn. A rim of light that hugs the glass under your pointer, and scanlines drifting up the screen. The rim is built from a box distance: q is how far the pixel sits outside the projected glass, so the glow is zero across the middle where the specimen is and brightest right at the edge. An earlier version drew a disc there instead, and it washed the specimen out. Change the numbers and save: 0.16 is how far the rim bleeds outward, 220 is how many scanlines fit the screen, and the vec3 is the colour of the whole field.",
      },
    ],
  },
  {
